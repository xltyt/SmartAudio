...
with torch.no_grad():
  ...
  hift = configs["hift"]
  hift.half()
  hift.load_state_dict(torch.load("%s/hift.fp16.pt" % target_model_dir, map_location=device), strict=True)
  hift.to(device).eval()
  print("Load Finished")
...
with torch.no_grad():
  with open(hyper_yaml_path, 'r') as f:
    configs = load_hyperpyyaml(f)
  hift = configs["hift"]
  hift.load_state_dict(torch.load("%s/hift.fp32.pt" % target_model_dir, map_location=device), strict=True)
  hift.to(device).eval()
  print("Load Finished")
  #script = torch.jit.optimize_for_inference(script)
